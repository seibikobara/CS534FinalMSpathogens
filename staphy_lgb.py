...
def lgboost_wo_Smote(features, outcome):
    # model performance
    acc_list=list()
    roc_list = list()
    ...
    for train_index, test_index in skf.split(features,outcome):
        ...
        # model 
        lgbm= lgb.LGBMClassifier(learning_rate=0.01,max_depth=-5,random_state=25)
...
def lgboost_Smote(features, outcome):
    # model performance
    acc_list=list()
    roc_list = list()
    skf = StratifiedKFold(n_splits=5)
...
for i in drugs:
    ...
    roc = lgboost_wo_Smote(x,y)
    res = res + [roc]
temp = pd.DataFrame(res)
result = temp.T
result.columns = drugs
result.to_csv('roc_staphy_lgb.txt', sep='\t',index=False)
...
for i in drugs:
    ...
    x = pd.read_csv(xfilename_)
    y = pd.read_csv(yfilename_)
    roc = lgboost_Smote(x,y)
    res = res + [roc]
temp = pd.DataFrame(res)
result_smote = temp.T
result_smote.columns = drugs
result_smote.to_csv('roc_staphy_lgb_smote.txt', sep='\t',index=False)
